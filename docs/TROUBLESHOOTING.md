...
기능이 아예 발동을 안 한 건 아니었다 — 계측해보니 한 판당 중반(돌 14~22개 시점)에 4~5회 실제로 트리거됨(예: 후보 67~79개 중 안전 판정 2~4개). 다만 이 판정이 진짜 정확한 위협 감지인지, 아니면 이 규모의 국면에서도 여전히 과대판정 성향이 있는지는 이번 세션에서 시간 관계상 끝까지 파고들지 못했다 — 후속 조사가 필요하면 이 지점(중반 14~22돌, 후보 대량 "위험" 판정)부터 시작하면 된다.
