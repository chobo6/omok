...
getAIMove(board, aiPlayer)
    ...
    ├─ 2. VCF(연속사 강제 승리) 탐색 → 강제 승리 수순이 있으면 그 첫 수 반환
    ...
    │        스스로 실패 처리하고 아래 방어로 넘어감)
    ├─ 3. 위급 방어 자리 계산(상대가 다음 수로 (a) 5목을 완성하거나 (b) 완성 지점이
    │     2곳 이상인 사를 만드는 자리) → 즉시 반환하지 않고 "강제 후보"로만 표시
    ├─ 4. 후보 수 생성 (기존 돌 주변 2칸 이내 빈 칸, 중복 제거)
    └─ 5. 반복심화(Iterative Deepening) + Transposition Table 탐색 (시간 예산 2초)
          → 3의 강제 후보는 MAX_CANDIDATES_PER_NODE에 밀려 잘리지 않도록 최우선 순위 고정,
            실제 선택은 탐색이 몇 수 앞을 보고 결정. 시간이 끝난 시점까지 완료된 depth 중
            가장 깊은 결과를 채택
```

> 3단계는 2026-07-04 이전엔 "즉시 반환"이었다(위급 방어 자리를 찾으면 탐색 없이 바로 그 수를 둠). 실전 패배 국면을 추적해보니 강한 상대(Yixin)와 대국할 때 백이 둔 수의 최대 70%가 이 반사 분기였다는 게 드러나 — 이러면 AI가 한 수 앞도 못 내다보고 방어만 반복하게 된다. `orderCandidates`/`rootSearch`/`iterativeDeepeningSearch`에 `forcedCells` 파라미터를 추가해, 위급 방어 자리는 "반드시 고려할 후보"로만 강제하고 실제 선택은 탐색이 하도록 변경했다.
...
- **금수/장목 인식 없음**: `aiEngine.js`는 렌주 금수(33/44/장목) 로직을 전혀 참조하지 않음(`server/forbidden.js`/`client/src/utils/forbidden.js`에 이미 구현된 `checkForbidden`과 완전히 분리). AI는 항상 백이라 자기 금수 걱정은 없지만, "상대(흑)의 유일한 방어 자리가 금수라 못 막는다" 같은 렌주 특유의 함정을 인식·유도하는 능력이 원천적으로 없음 — 2026-07-04 시점 미착수, 유력한 다음 후보
...
1. **금수/장목 활용** — 위 "알려진 한계" 참고. 지금까지 시도한 모든 항목과 근본적으로 다른(탐색 강화가 아닌 렌주 고유 지식) 전략 축이라 유력
2. **리프 평가 재도전 (더 큰 표본)** — 두 번의 시도가 실패했지만, 정확한 가중치를 찾으려면 후보당 20판보다 훨씬 큰 표본이 필요하다는 게 자동 튜닝 실험에서 확인됨(같은 가중치가 12판에선 9승3패, 20판에선 9승11패로 뒤집힘)
3. **네이티브 속도(WASM 컴파일)** — 지금까지 테스트한 배수(2~40배)에서 속도가 승률에 영향을 못 줬지만, WASM은 자릿수가 다른 속도 향상이라 완전히 배제하긴 이름. 다만 기대치는 낮춰야 함
4. **VCF-defend를 재귀적 카운터 탐색으로 확장** — 위 "VCF-defend는 1수 앞만 확인" 한계 해소
5. **난이도별 depth/시간예산 조절 기능 구현** — PRD 4절의 "AI 난이도 선택" 요구사항 반영. 실력과 무관하게 필요한 기능
6. **Killer Move / History Heuristic, PVS, VCT 재도전** — 전부 한 번 이상 시도 후 되돌린 항목. 원인이 해소되지 않는 한 재시도 우선순위 낮음
