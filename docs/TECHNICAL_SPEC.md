...
### 성능 개선 방향 (향후 과제, 우선순위 순 — Rapfi 리서치 기반)

1. **조합 패턴 평가표(`Pattern4`)** — 리프 평가를 윈도우 가중합에서 명시적 위협 유형 분류로 고도화, 위 "리프 평가가 윈도우 가중합 수준" 한계 해소
2. **VCT(사·삼을 함께 고려하는 확장 위협 탐색)** — 현재 VCF(사만 연속)보다 넓게, 열린 삼도 강제 수순에 포함시켜 종반 승률 추가 향상
3. **VCF-defend를 재귀적 카운터 탐색으로 확장** — 위 "VCF-defend는 1수 앞만 확인" 한계 해소, 완전한 Rapfi `vcfdefend`에 근접
4. **난이도별 depth/시간예산 조절 기능 구현** — PRD 4절의 "AI 난이도 선택" 요구사항 반영
5. **Killer Move / History Heuristic, PVS 재도전** — 위 한계에 적힌 원인(TT 상호작용 처리, 이 엔진에서의 낮은 기대 이득)을 해소할 방법이 있다면 재시도 가능
