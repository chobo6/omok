...
- **오프닝북(`openingBook.js`)**: 위 대각선 응수 4가지 각각에 대해, 로컬 Yixin에게 pbrain `BOARD` 명령으로 직접 질의해 얻은 백의 4/6수째 응수 8개를 저장. 국면을 "row,col,player" 정렬 문자열로 키를 만들어 정확히 일치할 때만 사용, 안 맞으면 정상 탐색으로 폴백
