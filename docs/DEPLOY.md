...
```
브라우저 (localhost, 80번 포트)
    │  kind가 호스트 80/443 → 노드 컨테이너로 직접 매핑(kind-config.yaml의 extraPortMappings)
    ▼
Ingress: omok-ingress (ingressClassName: nginx)
    ▼
ingress-nginx-controller (ingress-nginx 네임스페이스)
    ▼
...
`kubectl port-forward`는 더 이상 접속 경로로 쓰지 않는다(디버깅용으로만 필요시 사용). 대신 `kind-config.yaml`이 노드 생성 시점에 호스트의 80/443 포트를 노드 컨테이너로 매핑해두고, 그 안에서 ingress-nginx가 받아서 `omok-server` Service로 라우팅한다.
...
| `kind-config.yaml` (루트) | **쿠버네티스 리소스가 아니라 kind 도구 자체의 클러스터 설정**(노드 개수 등). `k8s/` 안에 두면 `kubectl apply -f k8s/`가 이것도 리소스로 착각해 에러 남 — 그래서 루트에 별도로 둠. ingress-nginx를 위해 호스트 80/443 포트 매핑(`extraPortMappings`)과 `ingress-ready=true` 노드 라벨을 포함 — 클러스터 생성 시점에만 적용되므로 이 파일을 바꾸면 클러스터를 지우고 다시 만들어야 함 |
...
| `k8s/30-ingress.yaml` | omok 네임스페이스로 들어오는 모든 경로(`/`)를 `omok-server` Service(4000번 포트)로 보내는 Ingress 규칙. `ingressClassName: nginx`로 ingress-nginx 컨트롤러를 지정 |
...
# 6. ingress-nginx 설치 (kind 전용으로 튜닝된 공식 매니페스트 — hostPort/ingress-ready 노드셀렉터 포함)
kubectl apply -f https://raw.githubusercontent.com/kubernetes/ingress-nginx/main/deploy/static/provider/kind/deploy.yaml
kubectl wait --namespace ingress-nginx --for=condition=ready pod \
  --selector=app.kubernetes.io/component=controller --timeout=120s

# 7. 접속 확인 (port-forward 불필요 — kind-config.yaml이 호스트 80번 포트를 직접 매핑해둠)
...
- **Ingress에 도메인/TLS 미설정** — 지금은 호스트 이름 구분 없이 `http://localhost`로 모든 경로(`/`)를 그대로 보내는 가장 단순한 규칙만 있음. HTTPS(TLS)도 아직 없음(로컬 실습이라 불필요)

## 10. 다음 단계

1. (완료) `ingress-nginx` 설치 → `k8s/30-ingress.yaml` 추가, port-forward 대신 `http://localhost`로 접속
2. (이후, 별도 단계) AWS EKS로 이전 — Postgres를 RDS로, Ingress 컨트롤러를 AWS Load Balancer Controller(진짜 ALB 생성)로, 이미지 배포를 ECR로 교체. 이 문서의 Dockerfile과 대부분의 `k8s/` 매니페스트(Ingress 리소스 형식 포함)는 그대로 재사용 가능하도록 설계함 (SSL 분기, PORT 환경변수화가 이를 위한 사전 작업)
